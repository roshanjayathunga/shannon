...
export default function Navbar() {
  ...
  const list = (anchor) => (
    ...
    ) : ( [{title:'About Us',path:'/about'}, {title:'Our Programmes',path:'/program'}, {title:'On stage and Beyond',path:'/onstagebeyond'}, {title:'Testimonials',path:'/testimonials'},{title:'Gallery',path:'/gallery'}, {title:'Contact Us',path:'/contact'}].map((item, index) => (
      <React.Fragment key={item.title}>
        <Slide direction="right" in={checked} mountOnEnter unmountOnExit
          style={{ transformOrigin: '0 0 0' }}
          {...(checked ? { timeout: 1000 * index / 4 } : {})}
